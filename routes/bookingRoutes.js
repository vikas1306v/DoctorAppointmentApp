...
router.put('/rejectBooking/:id', async (req, res) => {
    try {
        ...
        sendImmediateNotification(expoToken, false)
        return res.status(200).json({
...
router.put('/confirmBooking/:id', async (req, res) => {
    try {
        //check if slot is already booked or not ? pending ==> remaining
        const booking = await Booking.findByIdAndUpdate(req.params.id, {
            status: "approved",
            isBooked: true
        }, { new: true });
...
const sendNotification = async (booking) => {
    const user = await User.findById(booking.user);
    const expoToken = user.expoToken;
    sendImmediateNotification(expoToken, true)
...
const sendImmediateNotification = async (token, status) => {
    await fetch('https://exp.host/--/api/v2/push/send', {
        ...
        body: JSON.stringify({
            ...
            body: status ? "Your appointment has been successfully booked." : "Your appointment has been rejected.",
